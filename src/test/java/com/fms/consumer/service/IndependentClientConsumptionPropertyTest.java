...
class IndependentClientConsumptionPropertyTest {
     ...
     */
    @Property(tries = 100)
    void multiClient_independentWebSocketConnectionPerClientVehiclePair(
            @ForAll("vehicleSets") Set<Vehicle> vehicles,
            @ForAll("clientCounts") int clientCount) {
        ...
        // Verify each vehicle-client pair got its own createConnection call
        for (Vehicle vehicle : vehicles) {
            for (int i = 1; i <= clientCount; i++) {
        ...
        // Total connection creations should be vehicles * clients
        verify(clientPool, times(vehicles.size() * clientCount))
